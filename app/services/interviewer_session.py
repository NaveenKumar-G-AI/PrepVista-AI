...
async def create_session(
    ...
    async with DatabaseConnection() as conn:
        ...
        v2_runtime = {}
        if get_settings().INTERVIEW_ORCHESTRATOR_V2:
            from app.services.interview_orchestrator import build_blueprint, InterviewOrchestrator
            blueprint = build_blueprint(
                resume_summary, mode=interview_mode, duration=safe_duration,
                max_questions=cfg["max_turns"], difficulty=normalized_difficulty_mode, categories=categories,
            )
            safe_duration = blueprint.duration_seconds
            v2_runtime = {"orchestrator_v2": InterviewOrchestrator.create(
                blueprint, resume_summary, recent_memory.get("recent_questions", []),
            )}
            question_plan = [{"turn": i + 1, "category": "communication", "target": qid,
                              "max_questions": blueprint.max_questions}
                             for i, qid in enumerate(blueprint.planned_ids)]
    ...
    return {
        ...
        "department_code": _department_code,
        "blueprint": v2_runtime.get("orchestrator_v2", {}).get("blueprint"),
    }


async def process_answer(
    session_id: str, user_text: str, access_token: str,
    client_request_id: str | None = None, expected_turn: int | None = None,
    end_interview: bool = False, user_id: str | None = None,
) -> dict:
    """Commit one turn and its replay receipt atomically under a session row lock."""
    import hashlib
    import uuid
    try:
        uuid.UUID(str(session_id or ""))
    except (ValueError, AttributeError):
        return {"action": "error", "detail": "Invalid session ID format."}
    access_token = (access_token or "").strip()
    fingerprint = hashlib.sha256(_safe_json_dumps([user_text, end_interview, expected_turn]).encode()).hexdigest()
    async with DatabaseConnection() as conn:
        async with conn.transaction():
            session = await conn.fetchrow(
                """SELECT user_id, total_turns FROM interview_sessions
                   WHERE id = $1 AND access_token = $2 FOR UPDATE""",
                session_id, access_token,
            )
            if not session or (user_id is not None and str(session["user_id"]) != str(user_id)):
                return {"action": "error", "status": 404, "detail": "Interview session not found."}
            if client_request_id:
                receipt = await conn.fetchrow(
                    "SELECT fingerprint, response FROM interview_answer_receipts WHERE session_id = $1 AND request_id = $2",
                    session_id, client_request_id,
                )
                if receipt:
                    if receipt["fingerprint"] != fingerprint:
                        return {"action": "error", "status": 409, "detail": "This request key was already used for a different answer."}
                    return json.loads(receipt["response"]) if isinstance(receipt["response"], str) else dict(receipt["response"])
            if expected_turn is not None and expected_turn != int(session["total_turns"] or 0):
                return {"action": "error", "status": 409, "detail": "This question has already changed. Reload the interview before answering."}
            result = await _process_answer_in_transaction(conn, session_id, user_text, access_token, end_interview)
            if client_request_id and result.get("action") != "error":
                await conn.execute(
                    """INSERT INTO interview_answer_receipts (session_id, request_id, fingerprint, response)
                       VALUES ($1, $2, $3, $4)""",
                    session_id, client_request_id, fingerprint, _safe_json_dumps(result),
                )
            return result


async def _process_answer_in_transaction(conn, session_id: str, user_text: str, access_token: str, end_interview: bool = False) -> dict:
    session = await conn.fetchrow(
    ...
    if coerce_runtime_state(session["runtime_state"]).get("orchestrator_v2", {}).get("version") == 2:
        from app.services.interview_v2_session import process_v2
        return await process_v2(conn, session, user_text, end_interview)
    ...
    should_finish_after_close = end_interview
    closed_outcome: str | None = None

    if is_repeat_request and question_for_eval and not end_interview:
    ...
    if is_exit_request and not end_interview:
        if raw_user_text:
            await conn.execute(
    ...
    if raw_user_text and not is_greeting and not is_timeout and raw_user_text != "[USER_REQUESTED_END]":
        normalized_user_text = raw_user_text
        await conn.execute(
            """INSERT INTO conversation_messages (session_id, role, content, turn_number)
               VALUES ($1, 'user', $2, $3)""",
            session_id,
    ...
    elif is_timeout and not end_interview:
        ...
        if total_turns >= max_turn_limit:
            should_finish_after_close = True
        new_silence = 0
    elif not is_greeting and not raw_user_text and question_for_eval and not end_interview:
        return {
            "action": "continue",
...
async def finish_session(session_id: str, access_token: str, duration_actual: int | None = None) -> dict:
        ...
        async with conn.transaction():
            session = await conn.fetchrow(
                """SELECT id, user_id, plan, state, resume_summary, question_plan,
                          total_turns, duration_actual_seconds, runtime_state, final_score, rubric_scores, strengths, weaknesses
                   FROM interview_sessions
                   WHERE id = $1 AND access_token = $2 FOR NO KEY UPDATE""",
                session_id,
                access_token,
            )
            if not session:
            ...
            if session["state"] == "FINISHED":
                stored_result = coerce_runtime_state(session["runtime_state"]).get("finish_result_v2")
                if stored_result:
                    return stored_result
                return {
                    "final_score": session["final_score"],
                    "category_scores": json.loads(session["rubric_scores"]) if isinstance(session["rubric_scores"], str) else session["rubric_scores"],
                    "strengths": session["strengths"], "weaknesses": session["weaknesses"],
            ...
            if runtime_state.get("orchestrator_v2"):
                from app.services.interview_orchestrator import evidence_report
                state_v2 = runtime_state["orchestrator_v2"]
                state_v2["phase"] = "COMPLETED"
                ...
                runtime_state["finish_result_v2"] = {
                    ...
                    "weaknesses": result["weaknesses"], "total_questions": summary["closed_questions"],
                    "answered_questions": summary["answered_questions"], "expected_questions": summary["planned_questions"],
                    "completion_rate": summary["completion_rate"], "duration_seconds": effective_duration,
    ...
    if runtime_state.get("finish_result_v2"):
        return runtime_state["finish_result_v2"]
    neural_feedback = build_interview_neural_feedback(
        plan=str(session["plan"]),
    ...
    return {
        ...
        "evidence_report":    runtime_state.get("evidence_report_v2"),
