...
Uses the application's own `Settings` for the database URL (sync driver)
and `Base.metadata` for autogeneration, so migrations never drift from the
configuration and models the running application actually uses.
"""
