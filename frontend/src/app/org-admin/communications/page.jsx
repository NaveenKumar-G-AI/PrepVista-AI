...
const PRIORITY_META = {
  LOW: { label: "Low", color: "#94a3b8", tier: "Informational" },
  NORMAL: { label: "Normal", color: "#3b82f6", tier: "Normal" },
  HIGH: { label: "High", color: "#f59e0b", tier: "Important" },
  URGENT: { label: "Urgent", color: "#f43f5e", tier: "Critical" },
  CRITICAL: { label: "Critical", color: "#f43f5e", tier: "Critical" },
};
...
async function draftWithAI(instruction, facts) {
  // MOCKED FOR SAFETY
  return { 
    subject: "Update: " + (facts.company_name || "General"), 
    body: "This is a mocked AI draft for: " + (facts.company_name || "General") + ". Instruction was: " + instruction 
  };
}
...
function Card({ children, className, style }) {
  return (
    <div className={"rounded-xl border " + (className || "")} style={{ borderColor: "rgba(255,255,255,0.1)", backgroundColor: "#1e293b", ...(style || {}) }}>
      {children}
    </div>
  );
}
function Modal({ children, onClose }) {
  return (
    <div className="fixed inset-0 flex items-center justify-center p-4 z-50" style={{ backgroundColor: "rgba(22,35,63,0.45)" }} onClick={onClose}>
      <div className="rounded-xl p-5 w-full max-w-md max-h-full overflow-y-auto" className="bg-white/[0.02] border border-white/10" onClick={(e) => e.stopPropagation()}>
        {children}
      </div>
    </div>
  );
...
function FilterGroup({ label, children }) {
  return (
    <div className="mb-4">
      <div className="text-xs font-semibold uppercase tracking-wide mb-2" className="text-slate-400">{label}</div>
      <div className="flex flex-wrap gap-2">{children}</div>
    </div>
  );
}
function EmptyState({ icon: Icon, title, body, tone }) {
  const color = tone === "positive" ? "#10b981" : "#94a3b8";
  return (
    <Card className="p-8 flex flex-col items-center text-center">
      <Icon size={22} style={{ color }} />
      <div className="font-medium mt-3" className="text-white">{title}</div>
      <div className="text-sm mt-1 max-w-sm" className="text-slate-400">{body}</div>
    </Card>
  );
}
function LegendDot({ color, label }) {
  return (
    <span className="inline-flex items-center gap-1.5 text-xs" className="text-slate-400">
  ...
  if (recipient.status === "failed") {
    return (
      <div className="flex items-center gap-2">
        <div className="flex items-center justify-center w-7 h-7 rounded-full flex-shrink-0" style={{ backgroundColor: "#f43f5e" + "1A", border: "1.5px solid " + "#f43f5e" }}>
          <XCircle size={14} style={{ color: "#f43f5e" }} />
        </div>
        <span style={{ color: "#f43f5e", fontSize: 12 }}>FAILED \u2014 {recipient.failReason}</span>
      </div>
    );
  }
  ...
  return (
    <div className="flex items-center">
      {stages.map((s, i) => {
        ...
        return (
          <Fragment key={s.key}>
            {i > 0 && <div style={{ height: 1, width: 18, backgroundColor: done ? "#10b981" : "rgba(255,255,255,0.1)" }} />}
            <div className="flex flex-col items-center gap-1" style={{ minWidth: 44 }}>
              <div
                className="flex items-center justify-center w-7 h-7 rounded-full"
                style={{ backgroundColor: done ? "#10b981" + "1A" : COLORS.paper, border: "1.5px solid " + (done ? "#10b981" : "rgba(255,255,255,0.1)") }}
              >
                <Icon size={13} style={{ color: done ? "#10b981" : "#94a3b8" }} />
              </div>
              <span className="hidden sm:block" style={{ color: done ? "#ffffff" : "#94a3b8", fontSize: 9, textTransform: "uppercase", letterSpacing: "0.04em" }}>
                {s.label}
              </span>
            </div>
          </Fragment>
...
function DeliveryBar({ recipients, requiresAck }) {
  ...
  const segments = [
    { count: ackSeg, color: "#10b981" },
    { count: openedOnlySeg, color: "#3b82f6" },
    { count: deliveredOnlySeg, color: "#94a3b8" },
    { count: failed, color: "#f43f5e" },
  ].filter((s) => s.count > 0);

  return (
    <div>
      <div className="flex w-full rounded-full overflow-hidden" style={{ backgroundColor: "rgba(255,255,255,0.1)", height: 6 }}>
        {segments.map((s, i) => (
          <div key={i} style={{ width: (s.count / total) * 100 + "%", backgroundColor: s.color }} />
        ))}
      </div>
      <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2">
        <LegendDot color={"#94a3b8"} label={delivered + " delivered"} />
        <LegendDot color={"#3b82f6"} label={opened + " opened"} />
        {requiresAck && <LegendDot color={"#10b981"} label={acknowledged + " acknowledged"} />}
        {failed > 0 && <LegendDot color={"#f43f5e"} label={failed + " failed"} />}
      </div>
    </div>
  );
}
function StatCard({ label, value, sub, color, icon: Icon, onClick }) {
  return (
    <button onClick={onClick} className="text-left p-4 rounded-xl w-full" style={{ backgroundColor: "#1e293b", border: "1px solid " + "rgba(255,255,255,0.1)" }}>
      <Icon size={16} style={{ color }} />
      <div style={{ fontSize: 30, fontWeight: 700, color: "#ffffff", lineHeight: 1, marginTop: 8 }}>{value}</div>
      <div className="text-xs mt-1.5" className="text-white">{label}</div>
      {sub && <div className="text-xs mt-0.5" className="text-slate-400">{sub}</div>}
    </button>
  );
}
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div>
        ...
        <div className="text-xs" style={{ color: "#1e293b", opacity: 0.7 }}>{detail}</div>
      </div>
    </div>
  );
}
function TodayPanel({ student }) {
  ...
  return (
    <Card className="p-5" style={{ backgroundColor: "#ffffff", borderColor: "#ffffff" }}>
      <div style={{ color: "#1e293b", opacity: 0.6, fontSize: 11, textTransform: "uppercase", letterSpacing: "0.12em", marginBottom: 12 }}>Today</div>
      <div className="space-y-3">
        {upcomingInterview && <TodayItem color={"#f43f5e"} label={"Interview \u00B7 " + (drive ? drive.company : "")} detail={formatDateTime(upcomingInterview.date)} />}
        {deadlineDrive && <TodayItem color={"#f59e0b"} label={deadlineDrive.company + " application"} detail={"Closes " + formatDateTime(deadlineDrive.deadline)} />}
        {pendingOffer && <TodayItem color={"#10b981"} label={"Offer \u00B7 " + ((DRIVES.find((d) => d.id === pendingOffer.driveId) || {}).company || "")} detail={"Respond by " + formatDateTime(pendingOffer.deadlineToAccept)} />}
      </div>
    </Card>
  );
}
function RoleToggle({ role, setRole }) {
  return (
    <div className="inline-flex rounded-lg p-1" style={{ backgroundColor: "rgba(255,255,255,0.1)" + "80" }}>
      {[["tpo", "TPO desk"], ["student", "Student view"]].map(([id, label]) => (
        <button
          key={id}
          onClick={() => setRole(id)}
          className="px-3 py-1.5 rounded-md text-sm font-medium transition-colors"
          style={{ backgroundColor: role === id ? "#ffffff" : "transparent", color: role === id ? "#1e293b" : "#94a3b8" }}
        >
          {label}
        </button>
      ))}
...
function TPONav({ tab, setTab, openIssuesCount, failedCount }) {
  ...
  return (
    <div className="flex gap-1 overflow-x-auto" className="border-b border-white/10">
      {items.map(([id, label, count]) => (
        <button
          key={id}
          onClick={() => setTab(id)}
          className="px-3 py-2.5 text-sm font-medium whitespace-nowrap flex items-center gap-1.5 flex-shrink-0"
          style={{ color: tab === id ? "#ffffff" : "#94a3b8", borderBottom: "2px solid " + (tab === id ? "#ffffff" : "transparent") }}
        >
          {label}
          {count > 0 ? (
            <span className="rounded-full px-1.5" style={{ backgroundColor: "#f43f5e", color: "#1e293b", fontSize: 10 }}>{count}</span>
          ) : null}
        </button>
      ))}
    </div>
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={Bell} color={"#f59e0b"} value={needsReminder.length} label="need a drive reminder" sub={nearestDrive ? nearestDrive.company + " \u00B7 closes " + formatDateTime(nearestDrive.deadline) : "No drives closing soon"} onClick={() => setTpoTab("compose")} />
        <StatCard icon={ShieldCheck} color={"#f43f5e"} value={pendingAck} label="acknowledgments pending" sub="Critical messages awaiting confirmation" onClick={() => setTpoTab("sent")} />
        <StatCard icon={MessageSquare} color={"#3b82f6"} value={openIssuesCount} label="open student issues" sub={urgentIssuesCount + " urgent"} onClick={() => setTpoTab("issues")} />
        <StatCard icon={XCircle} color={"#f43f5e"} value={failedCount} label="failed deliveries" sub="Retry from the Failed tab" onClick={() => setTpoTab("failed")} />
      </div>
      <Card className="p-5">
        <h3 className="font-semibold mb-4 text-sm" className="text-white">Recent activity</h3>
        <div className="space-y-3">
            ...
            return (
              <div key={m.id} className="flex items-center justify-between gap-3 py-1.5">
                <div className="min-w-0">
                  <div className="text-sm font-medium truncate" className="text-white">{m.subject}</div>
                  <div className="text-xs mt-0.5" className="text-slate-400">{timeAgo(m.sentAt)}</div>
                </div>
                <div className="flex-shrink-0" style={{ color: "#94a3b8", fontSize: 12 }}>
  ...
  return (
    <Card className="p-5">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="font-semibold" className="text-white">Audience</h3>
      </div>
      ...
      {drive && (
        <div className="mb-4 p-2.5 rounded-lg" style={{ backgroundColor: COLORS.paper, color: "#94a3b8", fontSize: 12 }}>
      ...
      <div className="mt-5 pt-4 flex items-end justify-between flex-wrap gap-3" className="border-t border-white/10">
        <div>
          <div style={{ fontSize: 34, fontWeight: 700, color: "#ffffff", lineHeight: 1 }}>{matched.length}</div>
          <div className="text-xs mt-1" className="text-slate-400">{matched.length ? "students selected" : "pick a branch or drive to build an audience"}</div>
        </div>
        <div className="flex gap-3">
          {BRANCHES.filter((b) => breakdown[b] > 0).map((b) => (
            <div key={b} className="text-center">
              <div style={{ color: "#ffffff", fontSize: 13, fontWeight: 600 }}>{breakdown[b]}</div>
              <div className="text-xs" className="text-slate-400">{b}</div>
            </div>
          ))}
        </div>
      </div>
...
function Composer(props) {
  ...
  return (
    <Card className="p-5">
      <h3 className="font-semibold mb-4" className="text-white">Message</h3>

      <div className="mb-4">
        <label className="text-xs font-semibold uppercase tracking-wide" className="text-slate-400">Start from a template</label>
      ...
      <div className="mb-4 p-3 rounded-lg" className="bg-[#0f172a]">
        <div className="flex items-center gap-2 mb-2">
          <Sparkles size={14} style={{ color: "#3b82f6" }} />
          <span className="text-sm font-medium" className="text-white">Draft with AI</span>
        </div>
        <div className="flex gap-2">
          <input
            className="flex-1 min-w-0 px-3 py-2 rounded-lg text-sm outline-none"
            style={{ border: "1px solid " + "rgba(255,255,255,0.1)", backgroundColor: "#1e293b" }}
        ...
        {aiError && <div className="text-xs mt-2" style={{ color: "#f43f5e" }}>{aiError}</div>}
      </div>

      <div className="mb-4">
        <label className="text-xs font-semibold uppercase tracking-wide" className="text-slate-400">Subject</label>
        <input className="w-full mt-1 px-3 py-2 rounded-lg text-sm outline-none" className="border border-white/10" value={subject} onChange={(e) => setSubject(e.target.value)} />
      </div>

      <div className="mb-4">
        <label className="text-xs font-semibold uppercase tracking-wide" className="text-slate-400">Body</label>
        <textarea rows={4} className="w-full mt-1 px-3 py-2 rounded-lg text-sm outline-none" className="border border-white/10" value={body} onChange={(e) => setBody(e.target.value)} />
        <div className="flex flex-wrap gap-1.5 mt-2">
          {KNOWN_VARIABLES.map((v) => (
            <button key={v} onClick={() => setBody((b) => b + "{{" + v + "}}")} className="px-2 py-1 rounded" style={{ backgroundColor: COLORS.paper, color: "#94a3b8", border: "1px solid " + "rgba(255,255,255,0.1)", fontSize: 11 }}>
              {"{{" + v + "}}"}
            </button>
          ))}
        </div>
        {unknownVars.length > 0 && (
          <div className="text-xs mt-2" style={{ color: "#f43f5e" }}>Unknown variable{unknownVars.length > 1 ? "s" : ""}: {unknownVars.join(", ")}</div>
        )}
      </div>

      <div className="flex flex-wrap items-center gap-6 mb-4">
        <div>
          <label className="text-xs font-semibold uppercase tracking-wide block mb-1.5" className="text-slate-400">Priority</label>
          <div className="flex gap-1.5">
            {Object.keys(PRIORITY_META).map((id) => {
              const meta = PRIORITY_META[id];
              return (
                <button key={id} onClick={() => setPriority(id)} title={meta.label} className="w-7 h-7 rounded-full flex items-center justify-center" style={{ border: "2px solid " + (priority === id ? meta.color : "rgba(255,255,255,0.1)"), backgroundColor: priority === id ? meta.color + "26" : "transparent" }}>
        ...
        <label className="flex items-center gap-2 text-sm" className="text-white">
      ...
      <div>
        <label className="text-xs font-semibold uppercase tracking-wide block mb-2" className="text-slate-400">Channel</label>
        <div className="flex flex-wrap gap-2">
          <Badge color={"#10b981"}><Bell size={12} /> In-app \u2014 ready</Badge>
          <Badge color={"#94a3b8"}><Mail size={12} /> Email \u2014 not connected</Badge>
          <Badge color={"#94a3b8"}><Smartphone size={12} /> SMS \u2014 not connected</Badge>
          <Badge color={"#94a3b8"}><MessageCircle size={12} /> WhatsApp \u2014 not connected</Badge>
        </div>
      </div>

      {exampleStudent && (
        <div className="mt-5 pt-4" className="border-t border-white/10">
          <label className="text-xs font-semibold uppercase tracking-wide block mb-2" className="text-slate-400">Preview \u2014 {exampleStudent.name}</label>
          <div className="p-3 rounded-lg" className="bg-[#0f172a]">
            <div className="font-medium text-sm" className="text-white">{renderTemplate(subject, ctx) || "No subject"}</div>
            <div className="text-sm mt-1" className="text-slate-400">{renderTemplate(body, ctx) || "No message"}</div>
          </div>
        </div>
      )}
    </Card>
  ...
  return (
    <Modal onClose={onCancel}>
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="font-semibold" className="text-white">Confirm send</h3>
      </div>
      <p className="text-sm mb-4" className="text-slate-400">
        This message will reach <strong className="text-white">{count} student{count !== 1 ? "s" : ""}</strong>. Once sent, it can't be unsent \u2014 a correction would go out as a new message.
      </p>
      <div className="p-3 rounded-lg mb-4" className="bg-[#0f172a]">
        <div className="text-sm font-medium" className="text-white">{subject || "(no subject)"}</div>
      </div>
      <div className="flex justify-end gap-2">
        <button onClick={onCancel} className="px-4 py-2 rounded-lg text-sm font-medium" style={{ border: "1px solid " + "rgba(255,255,255,0.1)", color: "#ffffff" }}>Cancel</button>
        <button onClick={onConfirm} className="px-4 py-2 rounded-lg text-sm font-medium" style={{ backgroundColor: "#ffffff", color: "#1e293b" }}>Send to {count} student{count !== 1 ? "s" : ""}</button>
      </div>
    </Modal>
  );
}
    ...
    <Card className="p-4">
      <button className="w-full flex items-start justify-between gap-3 text-left" onClick={() => setOpen((o) => !o)}>
        <div className="flex items-start gap-3 min-w-0">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0" className="bg-[#0f172a]">
            <Icon size={15} className="text-white" />
          </div>
          <div className="min-w-0">
            <div className="font-medium text-sm truncate" className="text-white">{message.subject}</div>
            <div className="text-xs mt-0.5 flex items-center gap-2 flex-wrap" className="text-slate-400">
        ...
        <ChevronDown size={16} style={{ color: "#94a3b8", transform: open ? "rotate(180deg)" : "none", transition: "transform 0.15s", flexShrink: 0 }} />
      </button>
      <div className="mt-3">
        <DeliveryBar recipients={message.recipients} requiresAck={message.requiresAck} />
      </div>
      {open && (
        <div className="mt-4 pt-4" className="border-t border-white/10">
          <div className="text-sm mb-3" className="text-slate-400">{message.bodyTemplate}</div>
            ...
            {Object.entries(message.recipients).map(([sid, r]) => {
              ...
              return (
                <div key={sid} className="flex items-center justify-between gap-3 py-1.5 flex-wrap">
                  <span className="text-sm" className="text-white">{s ? s.name : sid}</span>
      ...
      {failed.map((f, i) => {
        ...
        return (
          <Card key={i} className="p-4 flex items-center justify-between gap-3 flex-wrap">
            <div>
              <div className="font-medium text-sm" className="text-white">{s ? s.name : f.studentId}</div>
              <div className="text-xs mt-0.5" className="text-slate-400">{f.message.subject}</div>
              <div className="text-xs mt-1" style={{ color: "#f43f5e"}}>{f.failReason}</div>
            </div>
            <button onClick={() => onRetry(f.message.id, f.studentId)} className="px-3 py-1.5 rounded-lg text-xs font-medium flex items-center gap-1.5" style={{ border: "1px solid " + "rgba(255,255,255,0.1)", color: "#ffffff" }}>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
      {TEMPLATES.map((t) => (
        <Card key={t.id} className="p-4">
          <div className="font-medium text-sm" className="text-white">{t.label}</div>
          <div className="text-xs mt-1" className="text-slate-400">{t.subject}</div>
          <button onClick={() => onUse(t.id)} className="mt-3 text-xs font-medium flex items-center gap-1" style={{ color: "#3b82f6" }}>Use template <ArrowRight size={12} /></button>
        </Card>
      ))}
    </div>
  );
...
function IssueDetail({ issue, student, onRespond }) {
  ...
  return (
    <div className="mt-4 pt-4 space-y-3" className="border-t border-white/10">
      <p className="text-sm" className="text-white">{issue.description}</p>
      <div>
        <label className="text-xs font-semibold uppercase tracking-wide block mb-1" className="text-slate-400">Status</label>
        <select value={status} onChange={(e) => setStatus(e.target.value)} className="px-3 py-2 rounded-lg text-sm outline-none" className="border border-white/10">
      ...
      <div>
        <label className="text-xs font-semibold uppercase tracking-wide block mb-1" className="text-slate-400">Response to {student ? student.name.split(" ")[0] : "student"}</label>
        <textarea rows={2} value={response} onChange={(e) => setResponse(e.target.value)} className="w-full px-3 py-2 rounded-lg text-sm outline-none" className="border border-white/10" />
      </div>
      <button onClick={() => onRespond(issue.id, status, response)} className="px-4 py-2 rounded-lg text-sm font-medium" style={{ backgroundColor: "#ffffff", color: "#1e293b" }}>Save</button>
    </div>
  );
}
              ...
              <Card key={i.id} className="p-4">
                <button className="w-full flex items-center justify-between text-left gap-3" onClick={() => setOpenIssueId((id) => (id === i.id ? null : i.id))}>
                  <div className="flex items-center gap-3 min-w-0">
                    <Badge color={(PRIORITY_META[i.priority] || {}).color || "#3b82f6"}>{i.priority}</Badge>
                    <div className="min-w-0">
                      <div className="font-medium text-sm truncate" className="text-white">{s ? s.name : i.studentId} \u00B7 {cat ? cat.label : i.category}</div>
                      <div className="text-xs mt-0.5" className="text-slate-400">{timeAgo(i.createdAt)} \u00B7 {i.status.replace(/_/g, " ")}</div>
                    </div>
                  </div>
...
function StatBlock({ label, value }) {
  return (
    <Card className="p-4">
      <div style={{ fontSize: 26, fontWeight: 700, color: "#ffffff" }}>{value}</div>
      <div className="text-xs mt-1" className="text-slate-400">{label}</div>
    </Card>
  );
}
function AnalyticsTab({ messages, issues }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="p-5">
        <h3 className="font-semibold mb-4 text-sm" className="text-white">Delivered vs. failed, recent messages</h3>
        <div style={{ width: "100%", height: 220 }}>
          <ResponsiveContainer>
            <BarChart data={chartData}>
              ...
              <Tooltip contentStyle={{ borderRadius: 8, border: "1px solid " + "rgba(255,255,255,0.1)", fontSize: 12 }} />
...
function NotificationCard({ message, student, onClick }) {
  ...
  return (
    <button onClick={onClick} className="w-full text-left p-4 rounded-xl flex items-start gap-3" style={{ backgroundColor: "#1e293b", border: "1px solid " + (unread ? meta.color : "rgba(255,255,255,0.1)"), borderLeftWidth: 4 }}>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          {unread && <span className="rounded-full flex-shrink-0" style={{ width: 6, height: 6, backgroundColor: meta.color }} />}
          <span className="font-medium text-sm truncate" className="text-white">{renderTemplate(message.subject, ctx)}</span>
        </div>
        <div className="text-sm mt-1 truncate" className="text-slate-400">{renderTemplate(message.bodyTemplate, ctx)}</div>
        <div className="mt-2" style={{ color: "#94a3b8", fontSize: 11 }}>
  ...
  return (
    <Modal onClose={onClose}>
      <div className="flex items-start justify-between mb-3 gap-3">
        <Badge color={PRIORITY_META[message.priority].color}>{PRIORITY_META[message.priority].label}</Badge>
        <button onClick={onClose} aria-label="Close"><X size={16} className="text-slate-400" /></button>
      </div>
      <h3 className="font-semibold mb-1" style={{ color: "#ffffff", fontSize: 20 }}>{renderTemplate(message.subject, ctx)}</h3>
      <p className="text-sm mb-4" className="text-slate-400">{renderTemplate(message.bodyTemplate, ctx)}</p>
      <div className="mb-4 overflow-x-auto">
        <StampTrail recipient={recipient} requiresAck={message.requiresAck} />
      </div>
      {message.requiresAck && !recipient.acknowledgedAt && (
        <button onClick={() => onAcknowledge(message.id, student.id)} className="w-full py-2.5 rounded-lg text-sm font-medium" style={{ backgroundColor: "#ffffff", color: "#1e293b" }}>Acknowledge</button>
      )}
      {recipient.acknowledgedAt && (
        <div className="flex items-center gap-1.5" style={{ color: "#10b981", fontSize: 12 }}><ShieldCheck size={13} /> Acknowledged {timeAgo(recipient.acknowledgedAt)}</div>
      )}
    </Modal>
  );
}
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3 gap-3">
        <h3 className="font-semibold text-sm" className="text-white">Your requests</h3>
        <button onClick={onNew} className="text-xs font-medium flex items-center gap-1 flex-shrink-0" style={{ color: "#3b82f6" }}><Plus size={13} /> Report an issue</button>
      </div>
      {!issues.length ? (
        <p className="text-sm" className="text-slate-400">Nothing reported. If something looks wrong \u2014 a broken link, a missing deadline \u2014 let the placement office know.</p>
      ) : (
        <div className="space-y-2">
          {issues.map((i) => (
            <div key={i.id} className="p-3 rounded-lg" className="bg-[#0f172a]">
              <div className="flex items-center justify-between gap-2">
                <span className="text-sm font-medium" className="text-white">{(ISSUE_CATEGORIES.find((c) => c.id === i.category) || {}).label}</span>
                <Badge color={i.status === "RESOLVED" || i.status === "CLOSED" ? "#10b981" : "#f59e0b"}>{i.status.replace(/_/g, " ")}</Badge>
              </div>
              <p className="text-xs mt-1" className="text-slate-400">{i.description}</p>
              {i.response && <p className="text-xs mt-2 pt-2" style={{ color: "#ffffff", borderTop: "1px solid " + "rgba(255,255,255,0.1)" }}><strong>Placement office:</strong> {i.response}</p>}
            </div>
          ))}
        </div>
      )}
...
function ReportIssueModal({ onClose, onSubmit }) {
  ...
  return (
    <Modal onClose={onClose}>
      <h3 className="font-semibold mb-4" className="text-white">Report an issue</h3>
      <label className="text-xs font-semibold uppercase tracking-wide block mb-1" className="text-slate-400">What's wrong?</label>
      <select value={category} onChange={(e) => setCategory(e.target.value)} className="w-full px-3 py-2 rounded-lg text-sm mb-3 outline-none" className="border border-white/10">
        {ISSUE_CATEGORIES.map((c) => <option key={c.id} value={c.id}>{c.label}</option>)}
      </select>
      <label className="text-xs font-semibold uppercase tracking-wide block mb-1" className="text-slate-400">Details</label>
      <textarea rows={3} value={description} onChange={(e) => setDescription(e.target.value)} className="w-full px-3 py-2 rounded-lg text-sm mb-4 outline-none" className="border border-white/10" placeholder="What happened?" />
      <div className="flex justify-end gap-2">
        <button onClick={onClose} className="px-4 py-2 rounded-lg text-sm font-medium" style={{ border: "1px solid " + "rgba(255,255,255,0.1)", color: "#ffffff" }}>Cancel</button>
        <button disabled={!description.trim()} onClick={() => onSubmit(category, description)} className="px-4 py-2 rounded-lg text-sm font-medium disabled:opacity-40" style={{ backgroundColor: "#ffffff", color: "#1e293b" }}>Submit</button>
      </div>
    </Modal>
  );
}
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-5">
      ...
      {tiers.map((tier) => grouped[tier].length > 0 && (
        <div key={tier}>
          <div className="text-xs font-semibold uppercase tracking-wide mb-2" className="text-slate-400">{tier}</div>
          <div className="space-y-2">
            {grouped[tier].map((m) => (
...
export default function CommunicationsPage() {
  ...
  return (
    <div className="min-h-screen w-full" style={{ backgroundColor: COLORS.paper, fontFamily: "'IBM Plex Sans', sans-serif" }}>
      <style>{"\n" + FONT_IMPORT + "\n* { box-sizing: border-box; }\ninput, textarea, select, button { font-family: inherit; }\n::selection { background: " + "#ffffff" + "; color: " + "#1e293b" + "; }\ninput:focus-visible, textarea:focus-visible, select:focus-visible, button:focus-visible {\n  outline: 2px solid " + "#ffffff" + ";\n  outline-offset: 2px;\n}\n@media (prefers-reduced-motion: reduce) {\n  * { animation-duration: 0.01ms !important; transition-duration: 0.01ms !important; }\n}\n"}</style>

      <header className="px-4 sm:px-6 lg:px-8 pt-6 pb-4" className="border-b border-white/10">
        <div className="max-w-5xl mx-auto flex flex-wrap items-center justify-between gap-4">
          <div>
            <div className="flex items-baseline gap-2 flex-wrap">
              <span style={{ fontWeight: 700, fontSize: 22, letterSpacing: "0.02em", color: "#ffffff" }}>PREPVISTA</span>
              <span style={{ color: "#94a3b8", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.1em" }}>Communication Center</span>
            </div>
            <div className="mt-1" style={{ color: "#94a3b8", fontSize: 11 }}>Prototype \u00B7 24-student mock roster \u00B7 in-app channel only</div>
          </div>
          <div className="flex items-center gap-2">
            <RoleToggle role={role} setRole={setRole} />
            {role === "student" && (
              <select value={currentStudentId} onChange={(e) => setCurrentStudentId(e.target.value)} className="px-3 py-2 rounded-lg text-sm outline-none" style={{ border: "1px solid " + "rgba(255,255,255,0.1)", backgroundColor: "#1e293b" }}>
