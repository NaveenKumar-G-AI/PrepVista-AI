...
import "./landing-styles/landing-page.css"
...
const rolePreviewVideos: Record<RoleId, string> = {
  software: "/films/software.mp4",
  data: "/films/data.mp4",
  core: "/films/core.mp4",
  analyst: "/films/business.mp4",
  unsure: "/films/notsure.mp4",
}
...
function CommunicationRound({ onComplete }: { onComplete: (result: RoundResult) => void }) {
  ...
  function startVoice(target: "first" | "retry") {
    ...
    recognition.onend = () => { if (recognitionRef.current === recognition) recognitionRef.current = null; setRecording(false) }
    recognition.onerror = () => { if (recognitionRef.current === recognition) recognitionRef.current = null; setRecording(false); setVoiceMessage("Voice recognition stopped. Your typed response is still available.") }
    ...
    try {
      ...
      recognitionRef.current = null
      setRecording(false)
      setVoiceMessage("Voice recognition could not start. Type your answer instead.")
    }
  }
  function stopVoice() {
    const recognition = recognitionRef.current
    ...
    try { recognition?.stop() } catch { /* It may already be stopped. */ }
    setRecording(false)
    setVoiceMessage("Recording stopped. Check the transcript before evaluation.")
  }
  function finish() {
...
function RolePreviewVideo({ role }: { role: Role }) {
  const videoRef = useRef<HTMLVideoElement | null>(null)
  ...
  return (
    <figure className="role-preview" aria-label={`${role.name} career visualization`}>
      <video
        key={rolePreviewVideos[role.id]}
        ref={videoRef}
        src={rolePreviewVideos[role.id]}
        autoPlay
        muted
        loop
        playsInline
...
export default function Home() {
  ...
  if (authLoading || user) {
    return <main className="min-h-screen bg-[#05070c]" aria-busy={authLoading} />
  }
...
function LandingExperience() {
  ...
  const [selectedRoleId, setSelectedRoleId] = useState<RoleId | null>("core")
  ...
  const [routeBreakVisible, setRouteBreakVisible] = useState(false)
  ...
  const journeyLineRef = useRef<HTMLElement | null>(null)
  const pageProgressRef = useRef<HTMLSpanElement | null>(null)
  const heroGhostRef = useRef<HTMLDivElement | null>(null)
  const journeyStageRef = useRef(0)
  const routeBreakRef = useRef(false)
  ...
  useEffect(() => {
    ...
    const motionObserver = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        ...
        target.dataset.motion = entry.isIntersecting ? "running" : "paused"
      })
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (pageProgressRef.current) pageProgressRef.current.style.transform = `scaleX(${pageProgress})`
      if (heroGhostRef.current) heroGhostRef.current.style.transform = `translate3d(${pageProgress * -70}px, 0, 0)`
      const section = journeyRef.current
      if (section) {
        const rect = section.getBoundingClientRect()
        const scrollable = Math.max(section.offsetHeight - window.innerHeight, 1)
        const progress = Math.min(1, Math.max(0, -rect.top / scrollable))
        if (journeyTrackRef.current) journeyTrackRef.current.style.transform = `translate3d(${progress * -74}%, -50%, 0)`
        if (journeyLineRef.current) journeyLineRef.current.style.transform = `scaleX(${progress})`

        const nextStage = Math.min(journeyGates.length - 1, Math.floor(progress * journeyGates.length))
        if (nextStage !== journeyStageRef.current) {
          journeyStageRef.current = nextStage
          setActiveStage(nextStage)
        }

        const showRouteBreak = progress > 0.72
        if (showRouteBreak !== routeBreakRef.current) {
          routeBreakRef.current = showRouteBreak
          setRouteBreakVisible(showRouteBreak)
        }
      }
    }
    const onScroll = () => {
  ...
  function trackCtaClick(location: string, cta: string) {
    void api.trackEvent("cta clicked", { location, cta })
  }

  function handleVote() {
    trackCtaClick("decision", "Vote for PrepVista")
    const configuredVoteUrl = process.env.NEXT_PUBLIC_STARTUPTHON_VOTE_URL?.trim()
    if (configuredVoteUrl) {
      try {
        const voteUrl = new URL(configuredVoteUrl)
        if (voteUrl.protocol === "https:" || voteUrl.protocol === "http:") {
          window.open(voteUrl.toString(), "_blank", "noopener,noreferrer")
          return
        }
      } catch {
        // Invalid configuration uses the same honest fallback as a missing URL.
      }
    }
    setVoteNotice(true)
  }
  ...
  function moveAmbient(event: React.PointerEvent<HTMLElement>) {
    ...
    ambientFrameRef.current = window.requestAnimationFrame(() => {
  ...
  return (
    <TooltipProvider delayDuration={160}>
      <main className={"v4-shell audience-" + audience} onPointerMove={moveAmbient}>
        ...
        <div className="page-progress" aria-hidden="true"><span ref={pageProgressRef} /></div>
        ...
        <header className="v4-nav">
          ...
          <nav aria-label="Primary navigation">
            ...
            <Link href="/pricing" onClick={() => trackCtaClick("header", "Pricing")}>Pricing</Link>
          </nav>
          <div className="v4-nav-actions">
            <span className="v4-live-state"><i /> INTERACTIVE PROTOTYPE</span>
            <Link className="v4-nav-signin" href="/login" onClick={() => trackCtaClick("header", "Sign In")}>Sign In</Link>
            <Button className="v4-nav-cta" onClick={() => { trackCtaClick("header", "Try a live round"); scrollToId("gauntlet") }}>Try a live round <Play /></Button>
          </div>
        </header>
        ...
        <section id="top" data-chapter="01 / YOUR FUTURE" className={"v4-hero " + (selectedRoleId ? "has-target" : "")}>
          ...
          <div ref={heroGhostRef} className="v4-hero-ghost" aria-hidden="true">BECOME</div>
          ...
          <div className="v4-hero-layout">
            <div className="v4-hero-copy reveal">
              ...
              <div className="v4-hero-actions">
                <Button className="v4-primary-cta" disabled={!selectedRoleId} onClick={() => { trackCtaClick("hero", "Enter my career route"); scrollToId("career-moment") }}>
                ...
                <Link className="v4-start-free" href="/login?mode=signup" onClick={() => trackCtaClick("hero", "Start Free Interview")}>Start Free Interview <ArrowUpRight /></Link>
              </div>
            ...
            <div className="v4-hero-film-slot reveal"><RolePreviewVideo key={role.id} role={role} /></div>
          </div>
        ...
        <section id="career-reality" ref={journeyRef} data-chapter="03 / PLACEMENT REALITY" className="v4-journey-world">
          <div className="v4-journey-sticky">
            ...
            <div className="v4-journey-stage">
              ...
              <div className="v4-journey-line" aria-hidden="true"><i ref={journeyLineRef} /></div>
              <div ref={journeyTrackRef} className="v4-gate-track">
              ...
              <div className={"v4-route-break " + (routeBreakVisible ? "is-visible" : "")}>
        ...
        <section id="career-fork" data-chapter="05 / TWO FUTURES" className="v5-fork-world">
          ...
          <div className="v5-fork-switch" role="group" aria-label="Compare two preparation futures">
            <button type="button" aria-pressed={futureMode === "unguided"} className={futureMode === "unguided" ? "is-active" : ""} onClick={() => setFutureMode("unguided")}><Split /> Future A · prepare by guesswork</button>
            <button type="button" aria-pressed={futureMode === "guided"} className={futureMode === "guided" ? "is-active" : ""} onClick={() => setFutureMode("guided")}><Waypoints /> Future B · prepare from evidence</button>
          </div>
          <div className={`v5-fork-stage mode-${futureMode}`}>
        ...
        <section id="vote" data-chapter="13 / THE DECISION" className="v4-vote-world">
          ...
          <div className="v4-vote-content reveal">
            ...
            <div className="v4-vote-actions">
              <Button className="v4-vote-primary" onClick={handleVote}>Yes — Vote for PrepVista <ArrowUpRight /></Button>
            ...
            <div className="v4-product-actions">
              <Link href="/login?mode=signup" onClick={() => trackCtaClick("decision", "Start Free Interview")}>Start Free Interview <ArrowRight /></Link>
              <Link href="/pricing" onClick={() => trackCtaClick("decision", "View Pricing")}>View Pricing</Link>
            </div>
        ...
        <footer className="v4-footer">
          ...
          <div className="v4-footer-mid">
            <div><span>EXPLORE</span><button onClick={() => scrollToId("gauntlet")}>Live experience</button><button onClick={() => scrollToId("career-proof")}>Career proof</button><button onClick={() => scrollToId("trust")}>Prototype status</button><Link href="/pricing" onClick={() => trackCtaClick("footer", "Pricing")}>Pricing</Link></div>
            ...
            <div><span>ACCOUNT & LEGAL</span><Link href="/login" onClick={() => trackCtaClick("footer", "Sign In")}>Sign In</Link><Link href="/login?mode=signup" onClick={() => trackCtaClick("footer", "Create Account")}>Create Account</Link><Link href="/privacy">Privacy Policy</Link><Link href="/terms">Terms</Link></div>
          </div>
