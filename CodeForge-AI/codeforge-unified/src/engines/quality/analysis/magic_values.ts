...
export function detectMagicValues(root: NormalizedNode): MagicValueHit[] {
  const numberCounts = new Map<number, { count: number; loc: NormalizedNode['loc'] }>();
  const stringCounts = new Map<string, { count: number; loc: NormalizedNode['loc'] }>();

  walk(root, (node) => {
    if (node.kind !== 'Literal') return;
    const literalType = node.meta?.literalType;
