...
5. Wire `src/service/codeforgeService.ts`'s methods to whatever HTTP/RPC layer
   the rest of PrepVista uses — they're already shaped as a clean API surface
   (§46) and don't assume any particular transport.
6. Build the actual student-facing UI (§40, §47-49) — explicitly out of scope
