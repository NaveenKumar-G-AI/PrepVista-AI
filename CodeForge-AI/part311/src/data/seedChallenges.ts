...
const CH1_DEDUPE: Challenge = {
  ...
  supportedLanguages: [SupportedLanguage.PYTHON, SupportedLanguage.JAVA],
  learningObjective:
  ...
  solutionMetadata: {
    referenceSolution: {
      ...
      [SupportedLanguage.JAVA]:
        ...
        "        Set<String> seen = new HashSet<>();\n" +
        "        List<List<Integer>> result = new ArrayList<>();\n" +
        "        for (List<Object> v : vectors) {\n" +
        "            List<Integer> intVec = new ArrayList<>();\n" +
        "            for (Object o : v) intVec.add(((Double) o).intValue());\n" +
        "            String key = intVec.toString();\n" +
        "            if (!seen.contains(key)) {\n" +
        "                seen.add(key);\n" +
        "                result.add(intVec);\n" +
