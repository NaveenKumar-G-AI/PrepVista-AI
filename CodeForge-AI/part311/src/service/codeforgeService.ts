...
export class CodeForgeNotFoundError extends Error {}
export class CodeForgeConflictError extends Error {}
...
export class CodeForgeService {
  ...
  saveDraft(attemptId: string, code: string): void {
    ...
    if (attempt.submittedAt) throw new CodeForgeConflictError("cannot edit an already-submitted attempt");
    this.store.updateAttempt(attemptId, { code, executionStatus: ExecutionStatus.DRAFT });
  }

  /**
   * §47's "run" vs "submit" distinction: quick feedback against PUBLIC tests
   * only, no grading, no profile update, no persisted evidence — safe to call
   * as often as the student likes while iterating. Does not require (or
   * mutate) an Attempt record at all, since nothing about it is evidence.
   */
  runPublicTests(challengeId: string, language: SupportedLanguage, code: string): { testsPassed: number; testsTotal: number; testResults: TestResult[] } {
    const challenge = this.requireChallenge(challengeId);
    const result = runDeterministicEvaluation(challenge, language, code, false);
    return { testsPassed: result.testsPassed, testsTotal: result.testResults.length, testResults: result.testResults };
  }

  requestHint(attemptId: string, level?: number): HintResult {
    const attempt = this.requireAttempt(attemptId);
    if (attempt.submittedAt) throw new CodeForgeConflictError("cannot request a hint on an already-submitted attempt");
  ...
  async submitAttempt(attemptId: string, code: string): Promise<SubmitResult> {
    const attempt = this.requireAttempt(attemptId);
    if (attempt.submittedAt) throw new CodeForgeConflictError(`attempt ${attemptId} was already submitted`);
    const challenge = this.requireChallenge(attempt.challengeId);
  ...
  private requireProfile(studentId: string): StudentProfile {
    ...
    if (!p) throw new CodeForgeNotFoundError(`no profile for student '${studentId}' — call ensureProfile() first`);
    return p;
  }

  private requireChallenge(challengeId: string): Challenge {
    const c = this.store.getChallenge(challengeId);
    if (!c) throw new CodeForgeNotFoundError(`no challenge '${challengeId}'`);
    return c;
  }

  private requireAttempt(attemptId: string): Attempt {
    const a = this.store.getAttempt(attemptId);
    if (!a) throw new CodeForgeNotFoundError(`no attempt '${attemptId}'`);
    return a;
  }
}
